...
def generate_uuid(version_uuid: int, name_space_dns: str = None) -> uuid.UUID:
    ...
    if version_uuid not in [1, 3, 4, 5]:
        raise errors_for_utils_data.UuidError("Invalid UUID version. Please provide a version between 1 and 5.")
    ...
    if version_uuid in [3, 5]:
        if name_space_dns is None:
            raise errors_for_utils_data.UuidNameSpaceDNSIsNotProvidedError(
