...
def generate_date(day: int = 0, month: int = 0, year: int = 0, hour: int = None, minute: int = None,
    ...
    if day > 28 and month == 2:
        raise errors_for_utils_data.DateGetError("Invalid day for February. Please provide a day between 1 and 28.")
    if day > 31:
        raise errors_for_utils_data.DateGetError("Invalid day. Please provide a day between 1 and 31.")
    if month > 12 or month < 1:
        raise errors_for_utils_data.DateGetError("Invalid month. Please provide a month between 1 and 12.")
    if hour is not None and (hour > 23 or hour < 0):
        raise errors_for_utils_data.DateGetError("Invalid hour. Please provide an hour between 0 and 23.")
    if minute is not None and (minute > 59 or minute < 0):
        raise errors_for_utils_data.DateGetError("Invalid minute. Please provide a minute between 0 and 59.")
    if second is not None and (second > 59 or second < 0):
        raise errors_for_utils_data.DateGetError("Invalid second. Please provide a second between 0 and 59.")
